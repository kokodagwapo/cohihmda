...
if (fs.existsSync(tractPipeline)) {
  ...
  if (src.includes('TRACT_CACHE_MAX')) ok('geo-tract-pipeline LRU cache')
...
if (fs.existsSync(hmdaDataTsx)) {
  ...
  if (src.includes("'lenders', path: 'lenders'") || src.includes('path: "lenders"')) {
    ok('HmdaData.tsx lenders tab uses /lenders path')
  } else fail('HmdaData.tsx lenders tab uses /lenders path')
  if (src.includes('<Outlet />') && !src.includes('VITE_HMDA_USE_IFRAME')) {
    ok('HmdaData.tsx native-only (no iframe fallback)')
  } else fail('HmdaData.tsx native-only (no iframe fallback)')
}
