...
function generateVersion() {
  try {
    ...
    // Get git commit SHA (full and short)
    const commitFull = getGitInfo('git rev-parse HEAD', null);
    const commitShort = commitFull ? commitFull.substring(0, 7) : null;

    // Get git tag (if any)
    const tag = getGitInfo('git describe --tags --exact-match HEAD 2>/dev/null', null);
    ...
    const ebVersionLabel = process.env.EB_VERSION_LABEL || process.env.VERSION_LABEL || null;
    ...
    if (!versionInfo.deployment.ebVersionLabel) delete versionInfo.deployment.ebVersionLabel;
