...
- The Cohi side panel is **closed by default** under the validator. Whenever an AC requires interacting with the panel (composer, chat transcript, clear/close controls) the plan MUST first click the floating dock chip `[data-testid="workbench-cohi-toggle"]` and only then assert/interact on the panel's internals. The panel body is not in the DOM while closed.
- **NEVER** target the Cohi toggle with `button:has-text("Cohi")`. On the canvas page the left sidebar renders buttons labeled "Cohi Insights" and "Cohi Mortgage News" (from `ReportsSidebar`), which appear BEFORE the workbench dock chip in DOM order. With `.first()` selection semantics, `button:has-text("Cohi")` will click the sidebar nav entry and navigate the browser away to `/insights`, destroying the workbench context for every subsequent step. Always use the `data-testid` locator.
- Workbench Cohi is **chat-first** (no Chat/Dashboards/Schema tabs). After opening the panel, target `[data-testid="workbench-cohi-panel"]` and the composer `textarea` (placeholder contains **canvas**). To add dashboard sections from the canvas, use the toolbar **Add** menu: click **Add** inside `#workbench-canvas-root`, then target category + section items with **page-scoped** locators — Radix dropdown content is **portaled** to `document.body`, so it is not a descendant of `#workbench-canvas-root`.
- The app auto-sends a proactive Cohi briefing shortly after load on canvases with widgets. The `WorkbenchCanvas` component **suppresses that auto-briefing** when the loaded canvas carries a `qaAgentRunTag` metadata field, which the QA fixture seeder stamps on every canvas it creates. In practice this means: when the plan is running against `testContext.seededCanvasUrl`, empty-state chat patterns such as **"Try asking"** are stable and you MAY assert on them on seeded canvases.
...
- For chat-panel readiness assertions that have to work both on and off seeded canvases, prefer stable controls: `[data-testid="workbench-cohi-panel"]`, **Cohi Insights** heading inside the panel, and/or the composer placeholder text (canvas-scoped).
