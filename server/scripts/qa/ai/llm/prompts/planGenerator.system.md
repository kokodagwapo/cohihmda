...
- The Cohi side panel is **closed by default** under the validator. Whenever an AC requires interacting with the panel (its Chat/Dashboards/Schema tabs, its composer, or its chat transcript) the plan MUST first click the toolbar toggle `[data-testid="workbench-cohi-toggle"]` and only then assert/interact on the panel's internals. The tabs are not in the DOM while the panel is closed.
- **NEVER** target the Cohi toggle with `button:has-text("Cohi")`. On the canvas page the left sidebar renders buttons labeled "Cohi Insights" and "Cohi Mortgage News" (from `ReportsSidebar`), which appear BEFORE the workbench toolbar's Cohi toggle in DOM order. With `.first()` selection semantics, `button:has-text("Cohi")` will click the sidebar nav entry and navigate the browser away to `/insights`, destroying the workbench context for every subsequent step. Always use the `data-testid` locator.
- Target the panel's tabs by testid as well — `[data-testid="workbench-cohi-tab-chat"]`, `[data-testid="workbench-cohi-tab-dashboards"]`, `[data-testid="workbench-cohi-tab-schema"]`. This is important because `button:has-text("Dashboards")` collides with the top nav's "Dashboards" dropdown menu trigger (rendered much earlier in the DOM), so `.first()` picks the nav menu instead of the Cohi panel tab — a `click` on that would just open the top nav dropdown, and an `assert ... toBeVisible` would silently pass against the wrong element (a false positive that violates the AC). `button:has-text("Chat")` and `button:has-text("Schema")` are currently unambiguous, but use the testids anyway for consistency and future-proofing.
