import { ALL_PRODUCTS, loadLenderPackFromDb } from './lender-warehouse-store.mjs'
    ...
    await Promise.all(
      codes.map(async (code) => {
        const { json } = await ffiecNationwideAggregations(
          { years: String(yr), loan_types: code, actions_taken: '1' },
          { timeoutMs: 20000 },
        )
        const count = Number(json?.aggregations?.[0]?.count) || 0
        if (count > 0) {
...
export async function buildProductSummaryFromDb(year = 2025) {
  ...
  const products = ALL_PRODUCTS.map((name) => {
    ...
    const topLenders = [...members]
      ...
      .slice(0, 5)
      .map((l) => l.name)
    return { name, count: members.length, unitsOriginated, topLenders }
  })
...
export async function getProductDimensionsForLenderFromDb(lei, year) {
  ...
  for (const r of rows) {
    ...
    grouped[r.dimension].push({ bucket: r.bucket, originations: r.originations })
  }
  return grouped
}
