...
- **isDataQuery: false** � No SQL. Use for: definitions ("what is FHA", "explain pull-through"), policies/procedures, product help, hypotheticals, or anything answerable from schema/metrics text alone without a portfolio query. Include a short **reason** string.
- **isDataQuery: true** � Return **sql** and a visualization that **directly answers** this specific question. Do not emit a generic chart "because we have data"; the chart type, title, ORDER BY, and LIMIT must match the user�s intent.
...
- **chartConfig.title** must state the metric, time window, and grain (e.g. "Top 10 loan officers by revenue � last 90 days (funded)").
