...
function getDateRanges(): {
  ...
  // Rolling 90 days
  const rolling90Start = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  const lastYearEnd = new Date(now.getFullYear() - 1, now.getMonth(), now.getDate()).toISOString().split('T')[0];
 ...
 */
async function fetchPredictions(tenantPool: pg.Pool): Promise<PredictionData[]> {
  try {
    ...
    // Get most recent prediction per loan
 ...
 */
async function fetchAtRiskVolume(tenantPool: pg.Pool, highRiskLoanIds: string[]): Promise<number> {
 ...
 */
async function fetchCreditRiskCount(tenantPool: pg.Pool): Promise<number> {
  try {
    const result = await tenantPool.query(`
 ...
 */
async function calculateRolling90DPullThrough(tenantPool: pg.Pool): Promise<number> {
 ...
 */
async function fetchLostOpportunity(tenantPool: pg.Pool, dateRange: DateRange): Promise<{
  withdrawnUnits: number;
  withdrawnVolume: number;
  withdrawnProformaRevenue: number;
  deniedUnits: number;
 ...
 */
async function fetchFunnelMetrics(tenantPool: pg.Pool, dateRange: DateRange): Promise<{
  loansStarted: number;
  loansLocked: number;
  loansOriginated: number;
  falloutRate: number;
...
async function calculateComparisons(
  ...
  try {
    ...
    const lastMonthMetrics = await queryMetrics(tenantPool, [
      'funded_volume',
      'avg_cycle_time',
      'pull_through_rate'
    ...
    const lastYearMetrics = await queryMetrics(tenantPool, [
      'funded_volume'
    ...
    const currentCycleTime = Number(currentMtdMetrics.avg_cycle_time?.value || 0);
    const lastMonthCycleTime = Number(lastMonthMetrics.avg_cycle_time?.value || 0);
    
    return {
      volumeVsLastMonth: lastMonthVolume > 0 ? ((currentVolume - lastMonthVolume) / lastMonthVolume) * 100 : 0,
      volumeVsLastYear: lastYearVolume > 0 ? ((Number(currentYtdMetrics.funded_volume?.value || 0) - lastYearVolume) / lastYearVolume) * 100 : 0,
      cycleTimeVsLastMonth: lastMonthCycleTime > 0 ? ((currentCycleTime - lastMonthCycleTime) / lastMonthCycleTime) * 100 : 0,
...
export async function collectInsightMetrics(
  ...
  console.log(`[InsightMetrics] Collecting metrics for dateFilter: ${dateFilter}`);
  ...
  const [
    ...
    // Predictions
    fetchPredictions(tenantPool),
    
    // Rolling 90D pull-through
    calculateRolling90DPullThrough(tenantPool),
    
    // Lost opportunity YTD
    fetchLostOpportunity(tenantPool, dateRanges.ytd),
    
    // Funnel YTD
    fetchFunnelMetrics(tenantPool, dateRanges.ytd),
    
    // High risk loan count
    fetchCreditRiskCount(tenantPool)
  ]);

  // Process predictions
  const withdrawPredictions = predictions.filter(p => p.predictedOutcome === 'withdraw');
  const denyPredictions = predictions.filter(p => p.predictedOutcome === 'deny');
  const originatePredictions = predictions.filter(p => p.predictedOutcome === 'originate');
  ...
  const totalAtRiskVolume = await fetchAtRiskVolume(tenantPool, highRiskLoanIds);
  ...
  const payload: InsightMetricsPayload = {
    ...
    performance: {
      ...
      revenueYTD: Number(ytdMetrics.total_revenue?.value || 0) || Number(ytdMetrics.funded_volume?.value || 0) * 0.01,
      revenueMTD: Number(mtdMetrics.total_revenue?.value || 0) || Number(mtdMetrics.funded_volume?.value || 0) * 0.01,
