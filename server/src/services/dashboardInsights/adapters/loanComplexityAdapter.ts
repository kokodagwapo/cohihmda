...
export const loanComplexityAdapter: DashboardAdapter = {
  ...
  async buildContext(
    ...
    return {
      ...
      pageDescription: PAGE_DESCRIPTION,
      pageGuidance: [
        "by_time_period keys are uppercase: MTD, QTD, YTD, LQ, LM, LY. Each period includes periodLabel, dateRange, summary, pivotSlices, barLoanOfficer, and by_current_loan_status. status_catalog is in data.summary.",
        "Chronology rule (critical): determine earlier vs later using period semantics/dateRange, not the order keys appear in by_time_period.",
        "Directional wording rule: when using 'from A to B' or 'improved vs', ensure A is earlier and B is later.",
        "Cohort: loans with application_date in the period. Bar chart uses mean complexity by group; pivot uses volume-weighted average (WA) complexity.",
        "Portfolio pull-through uses the SAME application-date cohort as complexity (not a funding-date cohort).",
        ...
        "Narratives should combine complexity moves with pull-through and/or outcome mix as context; describe co-movement and operational context, not causation.",
        "evidence_refs must use widget IDs from widget_catalog only: loan-complexity-bar-chart; loan-complexity-pivot-loan-officer; loan-complexity-pivot-processor; loan-complexity-pivot-underwriter; loan-complexity-pivot-closer; loan-complexity-pivot-branch; loan-complexity-pivot-current-loan-status.",
        "When claims reference groups, portfolio WA, or status mix, cite bar and/or pivot widgets as primary/supporting evidence.",
        "For a specific loan officer, use a widget with dimension complexity_loan_officer and target.label equal to the exact name from data.",
        "Populate filter_context.datePeriod as lowercase mtd|qtd|ytd|lq|lm|ly for every insight.",
        "For actor-focused insights (loan officer, processor, underwriter, closer), set filter_context.actor to the actor's name and keep it consistent with the primary evidence widget dimension.",
        "For branch-focused insights, set filter_context.branch to the exact branch name from data.",
        "Add channelGroup only when relevant. Do NOT reference Leaderboard or add leaderName; insights are scoped to Loan Complexity only.",
      ],
