 ...
 */
async function getActiveTenants(managementPool: pg.Pool): Promise<TenantInfo[]> {
...
async function runManagementMigrations(options: {
  ...
  try {
    const runner = new MigrationRunner(pool, 'management', MANAGEMENT_DB, options.verbose);
...
async function runTenantMigrations(
  ...
  try {
    const runner = new MigrationRunner(pool, 'tenant', tenant.database_name, options.verbose);
 ...
 */
async function showStatus(dbType: 'management' | 'tenant', tenantSlug?: string): Promise<void> {
  if (dbType === 'management') {
    ...
    try {
      const runner = new MigrationRunner(pool, 'management', MANAGEMENT_DB, true);
      const migrationsDir = getMigrationsDir('management');
  ...
  try {
    switch (command) {
      ...
      case 'up':
        ...
        const mgmtSuccess = await runManagementMigrations({ dryRun, targetVersion, verbose });
        if (!mgmtSuccess) {
          process.exit(1);
        }
        break;
      ...
      case 'tenant':
        ...
        const mgmtOk = await runManagementMigrations({ dryRun, targetVersion, verbose });
        if (!mgmtOk && !dryRun) {
          console.error('Management migrations must succeed before tenant migrations');
          process.exit(1);
        }
      ...
      case 'all':
        ...
        // Management first
        const allMgmtOk = await runManagementMigrations({ dryRun, targetVersion, verbose });
        if (!allMgmtOk && !dryRun) {
          process.exit(1);
        }
        ...
        try {
          ...
          console.log(`✓ Tenant databases: ${allTenantsList.length - allFailed}/${allTenantsList.length} OK`);
      ...
      case 'create':
        ...
        const migrationDbType = args.includes('--tenant') ? 'tenant' : 'management';
        ...
        const filename = `${nextVersion}_${migrationName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.sql`;
