...
import {
  getTenantFieldMappings,
  suggestFieldMappings,
  applyFieldMapping,
} from "../services/fieldMapper.js";
import {
 ...
 */
async function ensureLosConnectionsSchema(
  pool: any,
  tenantId?: string
): Promise<void> {
  try {
    ...
    if (!tableCheck.rows[0]?.exists) {
      logDebug("los_connections table does not exist, skipping migration", {
        tenantId,
      });
      return;
    }

    logInfo("Checking los_connections schema for missing columns", {
      tenantId,
    });
    ...
    if (!apiServerCheck.rows[0]?.exists) {
      logInfo("Adding encompass_api_server column to los_connections table", {
        tenantId,
      });
      await pool.query(`
    ...
    if (!foldersColumnCheck.rows[0]?.exists) {
      ...
      `);
      logInfo("Successfully added encompass_selected_folders column", {
        tenantId,
      });
    }
    ...
    if (!lastLoanModifiedCheck.rows[0]?.exists) {
      logInfo("Adding last_loan_modified_at column to los_connections table", {
        tenantId,
      });
      await pool.query(`
        ALTER TABLE public.los_connections 
    ...
    try {
      ...
      `);
      console.log(
        "[ensureLosConnectionsSchema] loan_officer_id migration check completed"
      );
    } catch (error: any) {
      console.error(
        "[ensureLosConnectionsSchema] Error migrating loan_officer_id:",
        error.message
      );
    ...
    logError("Error ensuring los_connections schema exists", schemaError, {
      tenantId,
...
const LOS_CONFIGS = {
  encompass: {
    ...
    documentation:
      "https://mortgagetech.ice.com/resources/encompass-developer-connect",
  },
  meridianlink: {
    name: "MeridianLink",
    authType: "api_key",
    ...
    documentation:
      "https://www.meridianlink.com/solutions/loan-origination-software/",
  },
  calyx: {
    name: "Calyx Point",
    authType: "database",
...
const losConnectionSchema = z.object({
  ...
  csv_upload_schedule: z
    .enum(["manual", "daily", "weekly", "hourly"])
    .optional(),
...
router.get("/types", authenticateToken, async (req: AuthRequest, res) => {
  ...
  if (shouldUseMockApi()) {
    Object.keys(types).forEach((losType) => {
      ...
      (config as any).mockApiUrl = `http://localhost:${
    ...
    try {
      if (!shouldUseMockApi()) {
        return res.status(400).json({
          ...
          message:
            "Set MOCK_LOS_API=true or run in development mode to use test connections",
        });
      }
      ...
      // Get tenant_id from request or JWT
      const tenantId =
      ...
      res.json({
        ...
        message:
          "Test connection created successfully. You can now test and sync with the mock API.",
      });
    } catch (error: any) {
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const tenantId =
        (req.query.tenant_id as string | undefined) || req.tenantId;
      ...
      if (tenantId) {
        try {
          ...
          logInfo("Ensuring los_connections schema is up to date", {
            tenantId,
          });
          try {
            ...
            logError(
              "Schema migration failed, cannot proceed with query",
              schemaError,
              { tenantId, error: schemaError.message }
            );
          ...
          try {
            ...
            if (tableError.code === "42P01") {
              logDebug("los_connections table does not exist for tenant", {
                tenantId,
              });
            ...
            if (
              ...
              logError(
                "Column missing after migration - this should not happen",
                tableError,
                { tenantId, error: tableError.message }
              );
              return res.status(500).json({
          ...
          logError("Error fetching LOS connections for tenant", error, {
            userId: req.userId,
            tenantId,
          });
      ...
      try {
        ...
        try {
          ...
          logError(
            "Schema migration failed, cannot proceed with query",
            schemaError,
            { tenantId: tenantContext.tenantId, error: schemaError.message }
          );
          return res.status(500).json({
            error: "Database schema migration failed",
            details: schemaError.message,
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      logError("Error fetching LOS connection", error, {
        userId: req.userId,
  ...
  async (req: AuthRequest, res) => {
    try {
      const schema = z.object({
        los_type: z.string(),
        encompass_instance_id: z.string().optional(),
        encompass_extraction_method: z
          .enum(["partner", "ropc", "api"])
          .optional(),
        api_client_id: z.string().optional(),
        api_client_secret: z.string().optional(),
        encompass_sa_username: z.string().optional(),
        encompass_sa_password: z.string().optional(),
        encompass_api_server: z.string().optional(),
      });
      ...
      if (data.los_type !== "encompass") {
        return res.json({
          success: true,
          message: "Credential testing not implemented for this LOS type",
        });
      }
      ...
      try {
        ...
        if (extractionMethod === "partner") {
          ...
          const basicAuth = Buffer.from(
            `${data.api_client_id}:${data.api_client_secret}`
          ).toString("base64");
          ...
          if (!data.encompass_sa_username || !data.encompass_sa_password) {
            return res.status(400).json({
              success: false,
              message:
                "SA Username and Password are required for ROPC/API flow",
            });
          }
          ...
          res.json({
            success: true,
            message:
              "Credentials format validated (full ROPC test not implemented)",
          });
        }
      } catch (testError: any) {
        const errorMessage =
      ...
      res
        .status(500)
        .json({ error: "Failed to test credentials", details: error.message });
    }
  }
);
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const tenantId =
        req.body.tenant_id || (req.query.tenant_id as string | undefined);
      ...
      try {
        ...
        logError("Error getting tenant pool", error, {
          userId: req.userId,
          tenantId,
        });
        return res.status(500).json({
      ...
      if (data.api_client_secret) {
        encryptedFields.api_client_secret_encrypted = await encryptField(
          data.api_client_secret
        );
      }
      if (data.encompass_sa_password) {
        encryptedFields.encompass_sa_password_encrypted = await encryptField(
          data.encompass_sa_password
        );
      }
      if (data.encompass_sa_username) {
        encryptedFields.encompass_sa_username_encrypted = await encryptField(
          data.encompass_sa_username
        );
      }
      if (data.api_client_id) {
        encryptedFields.api_client_id_encrypted = await encryptField(
          data.api_client_id
        );
      }
      if (data.db_password) {
        encryptedFields.db_password_encrypted = await encryptField(
          data.db_password
        );
      }
      ...
      RETURNING id, los_type, name, connection_method, api_environment, sync_enabled, sync_frequency, is_active, created_at`,
        [
          ...
          data.api_base_url ||
            LOS_CONFIGS[data.los_type]?.defaultBaseUrl ||
            null,
          data.api_key || null,
          data.api_environment || "sandbox",
          data.oauth_authorization_url ||
            (LOS_CONFIGS[data.los_type] as any)?.defaultAuthUrl ||
            null,
          data.oauth_token_url ||
            (LOS_CONFIGS[data.los_type] as any)?.defaultTokenUrl ||
            null,
          data.oauth_scopes ||
            (LOS_CONFIGS[data.los_type] as any)?.scopes?.join(" ") ||
            null,
          ...
          data.csv_field_mapping
            ? JSON.stringify(data.csv_field_mapping)
            : null,
          data.db_host || null,
          data.db_port || null,
          data.db_name || null,
      ...
      res.status(500).json({
        error: "Failed to create LOS connection",
        details: error.message,
      });
    }
  }
...
router.put(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      // Ensure schema is up to date (add missing columns) - MUST complete before query
      logInfo("Ensuring los_connections schema is up to date for update", {
      ...
      try {
        ...
        logError(
          "Schema migration failed, cannot proceed with update",
          schemaError,
          { tenantId: currentTenantId, error: schemaError.message }
        );
        return res.status(500).json({
          error: "Database schema migration failed",
          details: schemaError.message,
      ...
      const result = await tenantPool.query(
        `UPDATE public.los_connections SET ${updates.join(
          ", "
      ...
      logError("Error updating LOS connection", error, {
        userId: req.userId,
        connectionId: req.params.id,
        error: error.message,
        stack: error.stack,
      ...
      res.status(500).json({
        error: "Failed to update LOS connection",
        details: error.message,
      });
    }
  }
...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      console.log("[LOS Delete] Attempting to delete connection:", {
        connectionId: id,
        tenantId,
      });
    ...
    `);
      ...
      if (!tableCheck.rows[0]?.exists) {
        return res.status(404).json({
          error: "LOS connections table does not exist for this tenant",
        });
      }
      ...
      console.log("[LOS Delete] Connection check result:", {
        ...
        tenantId,
        allConnections: (
          await tenantPool.query("SELECT id FROM public.los_connections")
        ).rows.map((r) => r.id),
      });
      ...
      if (checkResult.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "LOS connection not found in tenant database" });
      }
      ...
      console.log("[LOS Delete] Delete result:", {
      ...
      logError("Error deleting LOS connection", error, {
        userId: req.userId,
        connectionId: req.params.id,
      });
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          const apiServer =
            connection.encompass_api_server || "https://api.elliemae.com";

          // Use extraction method from connection, default to 'partner' if not set
          const extractionMethod =
            connection.encompass_extraction_method || "partner";
          ...
          try {
            if (connection.api_client_id_encrypted) {
              apiClientId = await decryptField(
                connection.api_client_id_encrypted
              );
            ...
            if (connection.api_client_secret_encrypted) {
              apiClientSecret = await decryptField(
                connection.api_client_secret_encrypted
              );
            ...
            logError(
              "Failed to decrypt credentials for testing",
              decryptError,
              { userId: req.userId, connectionId: id }
            );
            return res.status(400).json({
              success: false,
              error:
                "Failed to decrypt credentials. Please check encryption configuration.",
            });
          }
          ...
          const tokenUrl = `${apiServer}/oauth2/v1/token`;
          console.log(
            "[LOS Test] Attempting OAuth token request to:",
            tokenUrl,
          ...
          if (extractionMethod === "partner") {
            ...
            if (!apiClientId || !apiClientSecret) {
              console.log("[LOS Test] Missing Partner credentials:", {
                hasClientId: !!apiClientId,
                hasClientSecret: !!apiClientSecret,
              });
              return res.status(400).json({
                success: false,
            ...
            if (
              ...
              console.log(
                "[LOS Test] ❌ Failed to obtain access token, status:",
            ...
            try {
              if (connection.encompass_sa_username_encrypted) {
                saUsername = await decryptField(
                  connection.encompass_sa_username_encrypted
                );
                console.log("[LOS Test] Successfully decrypted SA username");
              }
              if (connection.encompass_sa_password_encrypted) {
                saPassword = await decryptField(
                  connection.encompass_sa_password_encrypted
                );
                console.log("[LOS Test] Successfully decrypted SA password");
              }
            } catch (decryptError: any) {
              console.error(
                "[LOS Test] SA credential decryption error:",
                decryptError
              );
              return res.status(400).json({
                success: false,
                error:
                  "Failed to decrypt SA credentials. Please check encryption configuration.",
              });
            }

            if (!saUsername || !saPassword) {
              console.log("[LOS Test] Missing SA credentials:", {
                hasUsername: !!saUsername,
          ...
          if (testError.response) {
            ...
            errorMessage =
              "No response from Encompass API. Please check your network connection and API endpoint.";
            logError("Encompass connection test - no response", testError, {
              userId: req.userId,
              connectionId: id,
            });
          } else {
            ...
            logError(
              "Encompass connection test - request setup failed",
              testError,
              { userId: req.userId, connectionId: id }
            );
          }
        ...
        const hasConfig =
          connection.db_host && connection.db_name && connection.db_user;
        res.json({
          success: hasConfig,
          message: hasConfig
            ? "Database connection parameters configured"
            : "Database connection not fully configured",
          details: {
        ...
        res.json({
          ...
          message: hasPath
            ...
            : "CSV upload path not configured",
          details: {
      ...
      logError("Error testing LOS connection", error, {
        userId: req.userId,
        connectionId: req.params.id,
      });
      res
        .status(500)
        .json({ error: error.message || "Failed to test LOS connection" });
    }
  }
);
      ...
      if (
        ...
        if (clearDatabase) {
          ...
          try {
            ...
            logInfo("Loans database cleared", {
              connectionId: id,
              tenantId: tenantId,
            });
          } catch (error: any) {
            if (error.code === "42P01") {
              ...
              logInfo("Loans table does not exist, skipping clear", {
                connectionId: id,
                tenantId: tenantId,
              });
            } else {
        ...
        try {
          ...
          const uniqueLoansCount = parseInt(
            uniqueLoansResult.rows[0]?.unique_count || "0",
            10
          );

          console.log(
            `[Sync] Database loan counts - Total rows: ${loansCount}, Unique loan_ids: ${uniqueLoansCount}`
          );

          if (loansCount !== uniqueLoansCount) {
            console.warn(
              `[Sync] WARNING: Found ${
                loansCount - uniqueLoansCount
              } duplicate loan_ids in database!`
            );
          }
        } catch (error: any) {
          ...
          if (error.code === "42P01") {
            loansCount = 0;
            logInfo("Loans table does not exist, treating as empty", {
              connectionId: id,
              tenantId: tenantId,
            });
          } else {
        ...
        if (!fullSync && lastLoanModifiedAt && loansCount > 0) {
          ...
          try {
            ...
            if (maxModifiedResult.rows[0]?.max_modified) {
              ...
              logInfo(
                "No last_modified_date found in loans, will do full sync",
                { connectionId: id }
              );
            }
          } catch (queryError: any) {
            logWarn(
              "Could not query MAX(last_modified_date), will do full sync",
              {
                connectionId: id,
        ...
        if (!modifiedFrom) {
          logInfo(
            "Starting full sync (no Loan.LastModified filter, will use loanStartDate only)",
            {
              connectionId: id,
              tenantId: tenantId,
              existingLoansCount: loansCount,
              ...
              reason: !lastLoanModifiedAt
                ? "no last_loan_modified_at"
                : loansCount === 0
                ? "no existing loans"
                : "full sync requested",
            }
        ...
        logInfo("Final sync parameters", {
          ...
          tenantId: tenantId,
          modifiedFrom:
            modifiedFrom?.toISOString() || "undefined (no date filter)",
          fullSync: fullSync,
        });
        ...
        if (testMode && !syncLimit) {
          syncLimit = parseInt(
            process.env.ENCOMPASS_TEST_MODE_LIMIT || "50",
            10
          );
          logInfo("Test mode enabled: limiting sync", {
            connectionId: id,
            tenantId,
            limit: syncLimit,
          });
        ...
        const { processCSVFilesFromPath } = await import(
        ...
        res.json({
          success: true,
          message: "CSV processing started. Check sync logs for progress.",
        });
      } else {
        return res
          .status(400)
          .json({ error: "Sync not available for this connection method" });
      }
    } catch (error: any) {
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "Sync cancellation requested. The sync may take a moment to stop.",
      });
    } catch (error: any) {
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      try {
        ...
        logError("Error reading file buffer", bufferError, {
          userId: req.userId,
        ...
        return res.status(400).json({
          ...
          details:
            bufferError.message ||
            "Could not read the file contents. Please ensure the file is a valid CSV file.",
        });
      }
      ...
      try {
        ...
        logError("CSV parsing error", parseError, {
          userId: req.userId,
          fileName: req.file?.originalname,
        });
        return res.status(400).json({
          ...
          details:
            parseError.message ||
            "The CSV file format is invalid. Please check the file structure and try again.",
        });
      }
      ...
      if (criticalErrors.length > 0) {
        ...
        return res.status(400).json({
          ...
          suggestion:
            "Please check the CSV file format. Ensure all rows have the same number of columns as the header row.",
        });
      }
      ...
      if (!parseResult.data || parseResult.data.length === 0) {
        ...
        if (columns.length === 0) {
          return res.status(400).json({
            ...
            details:
              "Could not detect column headers in the CSV file. Please ensure the first row contains column names.",
          });
        }
      ...
      if (error.message?.includes("multer")) {
        ...
        errorDetails =
          "Please ensure the file is a valid CSV file and does not exceed 500MB.";
      } else if (
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!filePath || !connectionId) {
        return res
          .status(400)
          .json({ error: "File path and connection ID are required" });
      }
      ...
      logError("Error processing CSV", error, {
        userId: req.userId,
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      // Get tenant_id from JWT or query param
      let tenantId = (req.query.tenant_id as string) || req.tenantId;

      if (!tenantId) {
        // Try to get from management database profiles
        ...
        if (profileResult.rows.length > 0 && profileResult.rows[0].tenant_id) {
          ...
          return res
            .status(400)
            .json({ error: "Tenant ID required for demo upload" });
        }
      }

      logDebug("Using tenant for demo upload", {
        userId: req.userId,
        tenantId,
      });
      ...
      if (parsed.errors?.length) {
        logWarn("Demo CSV parse warnings", {
          userId: req.userId,
      ...
      for (let i = 0; i < records.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (record, batchIndex) => {
          ...
          try {
            ...
            if (tenantMappings) {
              mappedRecord = applyFieldMapping(
                record,
                tenantMappings.field_mappings
              );
            ...
            const getField = (patterns: string[], defaultValue?: any) => {
              for (const pattern of patterns) {
                if (
                  mappedRecord[pattern] !== undefined &&
                  mappedRecord[pattern] !== null &&
                  mappedRecord[pattern] !== ""
                ) {
            ...
            const loanData = {
              ...
              status: (() => {
                ...
                const lockDate = parseDate(
                  getField(["lock_date", "rate_lock_date"])
                );
              ...
              interest_rate: parseNumber(
                getField(["interest_rate", "rate", "apr", "note_rate"])
              ),
            ...
            const respaDate = parseDate(
              getField(["respa_date", "respaDate", "respa_application_date"])
            );
            const creditPullDate = parseDate(
              getField(["credit_pull_date", "creditPullDate", "credit_pull"])
            );
            const ficoScore = parseNumber(
              getField(["fico_score", "fico", "credit_score"])
            );
            const ltv = parseNumber(
              getField(["ltv", "loan_to_value", "loan_to_value_ratio"])
            );
            const loanPurpose = getField([
              "loan_purpose",
              "purpose",
              "loanPurpose",
            ]);
            const branch = getField(["branch", "branch_name", "office"]);
            const loanOfficerName = getField([
              "loan_officer_name",
              "loan_officer",
              "officer_name",
              "lo_name",
            ]);
            const falloutReason = getField([
              "fallout_reason",
              "falloutReason",
              "fallout",
            ]);
            const cycleTimeDays = parseNumber(
              getField(["cycle_time_days", "cycleTime", "cycle_time"])
            );
            const complexityScore = parseNumber(
              getField(["complexity_score", "complexityScore", "complexity"])
            ); // For TopTiering Ops scoring
              ...
              [
                ...
                cycleTimeDays ||
                  (loanData.application_date && loanData.closing_date
                    ? Math.round(
                        (new Date(loanData.closing_date).getTime() -
                          new Date(loanData.application_date).getTime()) /
                          (1000 * 60 * 60 * 24)
                      )
                    : null),
                ficoScore,
        ...
        if (records.length > 1000 && (i + BATCH_SIZE) % 1000 === 0) {
          logInfo(
            `Processed ${Math.min(i + BATCH_SIZE, records.length)} of ${
              records.length
            } records`,
            {
              userId: req.userId,
      ...
      res.json({
        ...
        message: `Processed ${recordsProcessed} loan records. ${
          recordsFailed > 0
      ...
      if (
        ...
        logError("Tenant creation may have failed", undefined, {
          userId: req.userId,
        });
        return res.status(500).json({
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    ...
    `);
      ...
      for (const row of columnsResult.rows) {
        ...
        `);
          ...
          const populatedCount = parseInt(
            countResult.rows[0]?.populated_count || "0"
          );
          ...
          logWarn(`Error calculating population for field ${columnName}`, {
            error: error.message,
          });
        }
      }
      ...
      const overallPopulationRate =
        fields.length > 0
          ? fields.reduce((sum, field) => sum + field.populationRate, 0) /
            fields.length
          : 0;
      ...
      logError("Error fetching field population stats", error, {
        userId: req.userId,
      });
      res.status(500).json({
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!losConnectionId) {
        return res
          .status(400)
          .json({ error: "connection_id query parameter required" });
      }
    ...
    `);
      ...
      for (const alias of allAliases) {
        ...
        `,
          ...
          if (colInfo.rows.length > 0) {
            ...
            const countResult = await tenantPool.query(
              `SELECT ${countQuery} as populated_count FROM public.loans`
            );
            populatedCount = parseInt(
              countResult.rows[0]?.populated_count || "0"
            );
            populationRate =
              totalLoans > 0 ? (populatedCount / totalLoans) * 100 : 0;
      ...
      `);
        ...
        if (
          sampleLoanResult.rows.length > 0 &&
          sampleLoanResult.rows[0].raw_data
        ) {
      ...
      for (const field of emptyFields) {
        ...
        if (foundVariations.length > 0) {
          ...
          const similarFields = sampleLoanFields.filter(
            (sf) =>
      ...
      const populatedFields = fieldMappings.filter(
        (f) => f.populationRate > 0
      ).length;
      ...
      res.json({
        ...
        allMappings: fieldMappings.sort(
          (a, b) => a.populationRate - b.populationRate
        ),
      });
    } catch (error: any) {
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    ...
    `);
      ...
      if (sampleLoanResult.rows.length === 0) {
        return res.json({
          error: "no_loans",
          message:
            "No loans found in database. Sync some loans to see transformation comparison.",
          summary: {
            totalRawDataKeys: 0,
            populatedColumns: 0,
            emptyColumns: 0,
    ...
    `);
      ...
      const fieldsKeys = rawDataKeys.filter(
        (k) => k.startsWith("Fields.") || k.startsWith("Loan.")
      );
      const otherKeys = rawDataKeys.filter(
        (k) => !k.startsWith("Fields.") && !k.startsWith("Loan.")
      );

      // Find populated columns (non-null values in sample loan)
      const populatedColumns = databaseColumns.filter(
        (col) => sampleLoan[col] !== null && sampleLoan[col] !== undefined
      );
      const emptyColumns = databaseColumns.filter(
        (col) => sampleLoan[col] === null || sampleLoan[col] === undefined
      );
      ...
      const findFieldInRawData = (
        fieldId: string
      ): { found: boolean; value: any; key: string } => {
        // Try exact match
        if (
          rawData[fieldId] !== undefined &&
          isValueMeaningful(rawData[fieldId])
        ) {
        ...
        if (fieldId.startsWith("Fields.")) {
          ...
          if (
            rawData[withoutPrefix] !== undefined &&
            isValueMeaningful(rawData[withoutPrefix])
          ) {
            return {
              found: true,
              value: rawData[withoutPrefix],
              key: withoutPrefix,
            };
          }
        }
        ...
        if (!fieldId.startsWith("Fields.")) {
          ...
          if (
            rawData[withPrefix] !== undefined &&
            isValueMeaningful(rawData[withPrefix])
          ) {
      ...
      for (const alias of allAliases.slice(0, 50)) {
        // Limit to first 50 for response size
        const fieldId = getDefaultFieldId(alias);
        const column = coheusAliasToColumnName(alias);
        const rawDataResult = fieldId
          ? findFieldInRawData(fieldId)
          : { found: false, value: undefined, key: "" };
        const hasDbValue =
          sampleLoan[column] !== null && sampleLoan[column] !== undefined;
      ...
      const problemMappings = expectedMappings.filter(
        (m) => m.hasRawData && !m.hasDbValue
      );
      ...
      logError("Error in transformation comparison", error, {
        userId: req.userId,
      });
      res.status(500).json({
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const { getAllCoheusAliases, coheusAliasToColumnName } = await import(
        "../services/encompassFieldMapper.js"
      );
    ...
    `);
      ...
      res.json({
        ...
        orphanedColumns: orphanedColumns.sort((a, b) =>
          a.column.localeCompare(b.column)
        ),
        missingColumns: missingColumns.sort((a, b) =>
          a.expectedColumn.localeCompare(b.expectedColumn)
        ),
        validColumns: validColumns.sort((a, b) =>
          a.column.localeCompare(b.column)
        ),
      });
    } catch (error: any) {
      logError("Error in schema-dictionary comparison", error, {
        userId: req.userId,
      });
      res.status(500).json({
