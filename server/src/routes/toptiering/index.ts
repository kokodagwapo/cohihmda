...
router.get('/', authenticateToken, attachTenantContext, apiLimiter, async (req: AuthRequest, res) => {
  try {
    ...
    logInfo('[TopTiering] Complete', {
      ...
      tiers: { top: topTierActors.length, second: secondTierActors.length, bottom: bottomTierActors.length }
    });
 ...
 */
router.get('/comparison', authenticateToken, attachTenantContext, apiLimiter, async (req: AuthRequest, res) => {
  try {
    const tenantPool = getTenantContext(req).tenantPool;
    ...
    try {
      ...
      logWarn('[TopTiering/Comparison] Failed to calculate YoY growth', { error: e });
    }
