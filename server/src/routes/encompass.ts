...
function getConnectionInfo(req: AuthRequest): {
  ...
  const losConnectionId =
    req.params.connectionId || req.body.losConnectionId || req.query.connectionId;
 ...
 */
router.post('/sync', authenticateToken, attachTenantContext, apiLimiter, async (req: AuthRequest, res) => {
  try {
    ...
    if (body.testMode && !syncLimit) {
      ...
      console.log(`[Encompass Sync] Test mode enabled: using limit of ${syncLimit} records`);
    }

    const result = await etlService.syncLoans(tenantId, losConnectionId, {
      fullSync: body.fullSync,
      modifiedFrom: body.modifiedFrom ? new Date(body.modifiedFrom) : undefined,
      limit: syncLimit,
      fields: body.fields,
 ...
 */
router.post('/sync-incremental', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    return res.status(501).json({ error: 'This endpoint needs to be updated for multi-tenant architecture' });
  } catch (error: any) {
    console.error('Error syncing Encompass loans:', error);
    res.status(500).json({ error: error.message || 'Failed to sync loans' });
  }
 ...
 * TODO: Update to use tenant-specific databases
 */
router.get('/sync-status/:connectionId', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    return res.status(501).json({ error: 'This endpoint needs to be updated for multi-tenant architecture' });
  } catch (error: any) {
 ...
 * TODO: Update to use tenant-specific databases
 */
router.get('/test-connection/:connectionId', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    return res.status(501).json({ error: 'This endpoint needs to be updated for multi-tenant architecture' });
  } catch (error: any) {
 ...
 */
router.get('/fields/:connectionId', (req, res, next) => {
  ...
  next();
}, authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    console.log('[Fields GET] Connection details:', { tenantId, losConnectionId });
    
    try {
      const rdbFieldsResponse = await apiService.getRdbFields(tenantId, losConnectionId);
      console.log('[Fields GET] Successfully fetched RDB fields:', rdbFieldsResponse.data.length);
      ...
      console.error('[Fields GET] Error fetching RDB fields from Encompass:', {
      ...
      if (apiError.response?.status === 401) {
        console.warn('[Fields GET] Authentication failed - returning empty RDB fields. Field mapping UI will work but without validation.');
        return res.json({
          rdbFields: [],
          concurrency: undefined,
          warning: 'Unable to authenticate with Encompass API. Field validation will be disabled.',
        });
      }

      // For other errors, still return empty array but log the error
      console.error('[Fields GET] Non-auth error fetching RDB fields:', apiError.message);
      return res.json({
        rdbFields: [],
        concurrency: undefined,
   ...
   */
  router.get('/folders/:connectionId', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      try {
        const foldersResponse = await apiService.getLoanFolders(tenantId, losConnectionId);
        console.log('[Folders GET] Successfully fetched folders:', foldersResponse.data.length);
        ...
        // Check if it's an authentication error
        if (apiError.response?.status === 401) {
          console.warn('[Folders GET] Authentication failed - returning empty folders.');
          return res.json({
            folders: [],
            concurrency: undefined,
            warning: 'Unable to authenticate with Encompass API. Folders cannot be loaded.',
          });
        }
 ...
 */
router.get('/field-swaps/:connectionId', (req, res, next) => {
  ...
  try {
    ...
    const swapsArray = Array.from(swaps.entries()).map(([alias, fieldId]) => ({
      coheusAlias: alias,
    ...
    res.status(500).json({ error: error.message || 'Failed to get field swaps' });
  }
});
 ...
 */
router.post('/field-swaps', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    const schema = z.object({
      ...
      swapType: z.enum(['Standard', 'Profitability']).optional().default('Standard'),
    });
    ...
    if (!tenantId) {
      return res.status(400).json({ error: 'tenant_id query parameter or body field is required' });
    }
    ...
    res.status(500).json({ error: error.message || 'Failed to save field swap' });
  }
});
 ...
 */
router.delete('/field-swaps/:connectionId', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    const schema = z.object({
      coheusAlias: z.string(),
    ...
    res.status(500).json({ error: error.message || 'Failed to delete field swap' });
  }
});
 ...
 */
router.get('/field-mappings', authenticateToken, apiLimiter, async (req: AuthRequest, res) => {
  try {
    console.log('[Field Mappings GET] Request received:', { userId: req.userId });
    ...
    const mappings = aliases.map((alias) => ({
      coheusAlias: alias,
      defaultEncompassFieldId: getDefaultFieldId(alias),
