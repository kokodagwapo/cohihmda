...
import { initSentry, sentryRequestHandler, sentryTracingHandler, sentryErrorHandler } from './middleware/sentry.js';
...
if (process.env.NODE_ENV !== 'production' || process.env.ENABLE_ENV_LOGGING === 'true') {
...
const allowedOrigins = (envOrigins ? `${envOrigins},${defaultOrigins}` : defaultOrigins)
  .split(',')
  .map(o => o.trim())
  .filter(o => o.length > 0)
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (origin === 'https://d2wvs4i87rs881.cloudfront.net' || origin.startsWith('https://d2wvs4i87rs881.cloudfront.net')) {
      return callback(null, true);
    }
...
const validateEnvironment = () => {
  ...
  for (const varName of requiredVars) {
    ...
    if (!value || value.trim() === '') {
      ...
      if (trimmed.length < 32) {
        console.error(`❌ ${varName} must be at least 32 characters long (current: ${trimmed.length})`);
        missing.push(varName);
      } else {
        // Check for common issues
        if (value.length !== trimmed.length) {
          warnings.push(`${varName} has leading/trailing whitespace (will be trimmed)`);
        }
        if (/\s/.test(trimmed)) {
          warnings.push(`${varName} contains internal whitespace - this may cause issues`);
        }
        // Check if it looks like a placeholder
        if (trimmed.toLowerCase().includes('change') || trimmed.toLowerCase().includes('secret') || trimmed === 'your_jwt_secret_min_32_chars') {
          warnings.push(`${varName} appears to be a placeholder value - please set a real secret`);
        }
      }
    }
  }
  ...
  if (missing.length > 0) {
    ...
    console.error('   Please set these variables in your Elastic Beanstalk environment configuration.');
    console.error('   In AWS: Go to Elastic Beanstalk → Environment → Configuration → Software → Environment properties');
    return false;
  }
...
const startServer = () => {
  ...
  if (!envValid) {
    console.error('⚠️  Continuing with invalid environment - authentication will fail');
  }
...
if (SKIP_DB) {
  ...
  initDatabase().then(async () => {
    ...
    if (NODE_ENV !== 'test') {
      try {
        const { startSyncScheduler } = await import('./services/losSyncScheduler.js');
        startSyncScheduler();
      } catch (error) {
      ...
      try {
        const { startVendorSyncScheduler } = await import('./services/vendorSyncScheduler.js');
    ...
    console.error('⚠️ Starting server without database (some features may be unavailable)');
