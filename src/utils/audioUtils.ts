...
export class AudioQueue {
  ...
  private async playNext() {
    ...
    try {
      ...
      // Add small gap between chunks to prevent clicking
      const gapDuration = 0.005; // 5ms gap
      const startTime = Math.max(this.audioContext.currentTime, this.nextStartTime + gapDuration);
      ...
      // Apply subtle fade at start and end to prevent clicks
      const fadeTime = 0.003; // 3ms fade
      gainNode.gain.setValueAtTime(0, startTime);
      gainNode.gain.linearRampToValueAtTime(1, startTime + fadeTime);
      gainNode.gain.setValueAtTime(1, startTime + audioBuffer.duration - fadeTime);
