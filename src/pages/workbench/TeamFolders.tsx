    ...
    try {
      ...
      if (typeof window !== 'undefined' && isUnifiedChatClientEnabled()) {
        answer = await postUnifiedWorkbenchHubQuery(
          prompt,
          undefined,
          WORKBENCH_HUB_SCOPE_IDS.teamFolders,
        );
      } else {
            ...
            <div className="max-w-[1600px] mx-auto">
              <div className="mb-8 sm:mb-10">
                <div className="flex items-start gap-4">
                  <IconBadge icon={Users} variant="slate" size="xl" rounded="2xl" />
                  <div>
                    <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-slate-100 tracking-tight">
                      Team Folders
                    </h1>
                    <p className="mt-1.5 text-[15px] text-slate-600 dark:text-slate-400 max-w-xl">
                      Shared team spaces to organize dashboards, reports, and collaboration notes.
                    </p>
                    <Link
                      to="/my-dashboard"
              ...
              <div className="rounded-2xl border border-dashed border-slate-200/80 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/70 p-6 sm:p-8 text-center">
                <IconBadge icon={Folder} variant="slate" size="lg" rounded="xl" className="mx-auto mb-3" />
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">No team folders yet</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  Create a shared folder to organize team dashboards and reports.
                </p>
              </div>
            </div>
          </div>
