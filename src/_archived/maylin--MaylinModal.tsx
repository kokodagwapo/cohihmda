...
interface MaylinModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function MaylinModal({ isOpen, onClose }: MaylinModalProps) {
  ...
  const startConnection = async () => {
    try {
      ...
      wsRef.current = api.createWebSocket('maylin-realtime');
  ...
  const toggleMute = () => {
    ...
    toast({
      ...
      description: newMutedState ? 'Cam cannot hear you' : 'Cam can hear you now',
    });
  };
  ...
  const cleanup = () => {
    console.log('Cleaning up Cam connection...');
