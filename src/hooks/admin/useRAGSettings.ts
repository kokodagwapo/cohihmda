...
export const useRAGSettings = () => {
  ...
  const loadRagVoiceData = useCallback(async (useCache: boolean = true, tenantId?: string | null) => {
    ...
    if (useCache && !tenantId) {
      ...
      if (cached && cacheTimestamp && (now - parseInt(cacheTimestamp)) < cacheTTL) {
        try {
    ...
    try {
      ...
      const [settingsData, costsData] = await Promise.all([
        api.request<{ settings: any }>(`/api/rag/settings${params.toString() ? '?' + params.toString() : ''}`),
        api.request<{ costs: any[] }>(`/api/rag/costs${params.toString() ? '?' + params.toString() : ''}`),
      ]);
  ...
  const saveRagVoiceSettings = useCallback(async (settings: any) => {
    try {
  ...
  const saveApiKeys = useCallback(async (openaiKey: string, geminiKey: string) => {
    try {
      await api.request('/api/rag/api-keys', {
        method: 'PUT',
