...
export const getWebSocketUrl = (): string => {
  ...
  if (import.meta.env.VITE_WEBSOCKET_URL && import.meta.env.VITE_WEBSOCKET_URL.trim() !== '') {
  ...
  if (typeof window !== 'undefined') {
    const customBackendUrl = localStorage.getItem('BACKEND_API_URL');
    if (customBackendUrl && customBackendUrl.trim() !== '') {
      console.log('Using direct backend URL for WebSocket from localStorage:', customBackendUrl);
      return customBackendUrl;
    }
    ...
    if (hostname.includes('cloudfront.net')) {
      ...
      if (apiUrl && apiUrl.trim() !== '' && !apiUrl.includes('cloudfront.net')) {
      ...
      const errorMsg = 'WebSocket connection requires direct backend URL. CloudFront does not support WebSocket connections.\n\n' +
        'To fix this:\n' +
      ...
      throw new Error('WebSocket backend URL not configured. Please set BACKEND_API_URL in localStorage or VITE_WEBSOCKET_URL environment variable.');
    }
  }
...
export class ApiClient {
  ...
  async invokeFunction<T>(functionName: string, body: any): Promise<T> {
    ...
    if (!apiGatewayUrl) {
      throw new Error('API Gateway REST URL not configured. Set VITE_API_GATEWAY_REST_URL');
    }
  ...
  createWebSocket(path: string): WebSocket {
    ...
    if (!apiGatewayWsUrl) {
      throw new Error('API Gateway WebSocket URL not configured. Set VITE_API_GATEWAY_WEBSOCKET_URL');
    }
  ...
  async request<T>(endpoint: string, options: RequestInit = {}, retries = 0): Promise<T> {
    ...
    if (retries > 0 || import.meta.env.DEV) {
      console.log(`[API Request] ${options.method || 'GET'} ${endpoint} -> ${url} (retry ${retries})`);
    }
    
    // Skip caching for non-GET requests or if cache-busting is requested
    const isGetRequest = !options.method || options.method === 'GET';
    const skipCache = options.headers && 'Cache-Control' in options.headers && (options.headers as any)['Cache-Control'] === 'no-cache';
    ...
    const requestPromise = this.executeRequest<T>(endpoint, url, options, retries, cacheKey, isGetRequest, skipCache);
  ...
  private async executeRequest<T>(
    ...
    const isSlowEndpoint = endpoint.includes('/loans/funnel') || 
                          endpoint.includes('/dashboard/analytics') || 
                          endpoint.includes('/dashboard/insights');
    const timeoutMs = (isFileUpload || isImportEndpoint) ? 600000 : (isSlowEndpoint ? 60000 : 30000); 
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorData = { 
            error: responseText && responseText.trim() 
        ...
        if (response.status === 503) {
          throw new Error(errorData.error || 'Service temporarily unavailable. Please try again.');
        }
      ...
      if (!responseText || !responseText.trim()) {
        ...
        throw new Error(`Server returned empty response (status ${response.status}). This may indicate a server error.`);
      }
      ...
      try {
        ...
        if (parseError.message?.includes('Unexpected end of JSON input')) {
          throw new Error(`Server returned empty or incomplete JSON response. This may indicate the server crashed or encountered an error while processing your request. Status: ${response.status}. Response preview: ${responseText.substring(0, 100)}`);
        }
        throw new Error(`Failed to parse server response as JSON: ${parseError.message}. Response preview: ${responseText.substring(0, 100)}`);
      }
      ...
      if (error.name === 'AbortError' || error.message?.includes('timeout')) {
        ...
        if (retries < 1) {
          console.warn(`Request timeout for ${endpoint} after ${timeoutDuration}. Retrying... (attempt ${retries + 1}/1)`);
          await new Promise(resolve => setTimeout(resolve, 2000));
          return this.executeRequest(endpoint, url, options, retries + 1, cacheKey, isGetRequest, skipCache);
        }
        // Provide more helpful error message for slow endpoints
        const baseUrlInfo = this.baseUrl || 'CloudFront proxy';
        if (isSlowEndpoint) {
          throw new Error(`Request timed out after ${timeoutDuration}. This endpoint may be processing a large dataset. The backend may be slow or unavailable. Please try again in a moment.`);
        }
        console.error(`Request timed out after retries. Endpoint: ${endpoint}, Base URL: ${baseUrlInfo}, Timeout: ${timeoutDuration}`);
        throw new Error(`Request timed out after ${timeoutDuration}. The server at ${baseUrlInfo} may be slow or unavailable. Please check your connection and try again.`);
      }
      ...
      if (
        ...
        console.warn(`Connection error to ${url}. Retrying in 2 seconds... (attempt ${retries + 1}/2)`);
        await new Promise(resolve => setTimeout(resolve, 2000));
        return this.executeRequest(endpoint, url, options, retries + 1, cacheKey, isGetRequest, skipCache);
      }
      ...
      if (isConnectionError) {
        ...
        if (endpoint !== '/health' && endpoint !== '/api/health') {
          try {
            ...
            if (healthCheck.ok || healthCheck.status === 503) {
              ...
              throw new Error(`Unable to complete request to ${endpoint}. The server is running. Please check your authentication or try again.`);
            }
            ...
            if (healthError.name === 'AbortError' || healthError.name === 'TypeError') {
              ...
              if (healthError.message?.includes('CORS') || healthError.message?.includes('blocked by CORS policy')) {
                throw new Error(`CORS Error: Unable to verify server connection. The backend may not be configured to allow requests from ${window.location.origin}. Please update the backend FRONTEND_URL environment variable.`);
              }
              const serverLocation = this.baseUrl || 'the backend server';
              throw new Error(`Unable to connect to ${serverLocation}. The server did not respond. Please ensure the backend server is running.`);
            }
        ...
        if (isCorsError) {
          const serverLocation = this.baseUrl || 'the backend server (via CloudFront proxy)';
          throw new Error(`CORS Error: ${serverLocation} is not configured to allow requests from ${window.location.origin}. Please update the backend FRONTEND_URL environment variable to include this origin.`);
        }
        
        const serverLocation = this.baseUrl || 'the backend server';
        throw new Error(`Unable to connect to ${serverLocation}. Please ensure the backend server is running.`);
      }
  ...
  async signIn(email: string, password: string) {
    ...
    try {
      ...
      if (healthError.name === 'AbortError') {
        serverError = 'Server did not respond. Please ensure the backend server is running on port 3001.';
      } else {
        serverError = 'Unable to connect to server. Please ensure the backend server is running on port 3001.';
      }
    }

    if (!serverReachable) {
      throw new Error(serverError || 'Unable to connect to server. Please ensure the backend server is running on port 3001.');
    }

    try {
      const data = await this.request<{ user: any; token: string }>('/api/auth/signin', {
        method: 'POST',
      ...
      if (error.message?.includes('503') || 
        ...
        try {
          ...
          if (retryError.message?.includes('503') || retryError.message?.includes('Service temporarily unavailable')) {
            throw new Error('Service temporarily unavailable. The database connection is down. Please try again in a moment.');
          }
      ...
      if (errorMsg.includes('failed to fetch') || 
        ...
        try {
          ...
          if (checkResponse.ok || checkResponse.status === 503) {
            throw new Error('Server is running but the request failed. Please try again.');
          }
        } catch (checkError) {
  ...
  // News methods
  async getNews() {
    return this.request<{ newsFeed: any[]; lastUpdated: string; error?: string }>('/api/news');
  }
  ...
  createBackendWebSocket(path: string): WebSocket {
    ...
    if (!storedToken || storedToken.trim() === '') {
      if (import.meta.env.PROD) {
        throw new Error('Authentication required: please sign in before starting a live voice session.');
      }
    }
    ...
    if (isHttpsPage && backendIsHttp && wsProtocol === 'wss://') {
      console.warn('⚠️ Attempting wss:// connection to HTTP backend. If this fails, configure HTTPS listener on ALB.');
    }
    ...
    console.log('🔌 Creating WebSocket connection:', {
      ...
      note: isHttpsPage && backendIsHttp 
    ...
    try {
      ...
      if (error?.message?.includes('insecure') || 
        ...
        const errorMsg = 'WebSocket connection blocked: HTTPS page requires secure WebSocket (wss://). ' +
    ...
    ws.addEventListener('error', (error: any) => {
      ...
      if (isSecurityError || (isHttpsPage && wsProtocol === 'wss://' && backendUrl.startsWith('http://'))) {
        const errorMsg = 'WebSocket connection blocked: HTTPS page requires secure WebSocket (wss://). ' +
          'The Application Load Balancer needs an HTTPS listener configured with an SSL certificate. ' +
          'See ALB_HTTPS_SETUP.md for instructions.';
        ...
        console.error('❌ WebSocket Security Error:', {
          ...
          solution: 'Configure HTTPS listener on ALB with SSL certificate from AWS Certificate Manager'
        });
