  ...
  const filteredLoans = searchQuery
    ? loans.filter(
        (loan) =>
          ...
          loan.current_loan_status?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-sm text-slate-600 dark:text-slate-400 font-light">
              Explore loan records in the tenant database ({total.toLocaleString()} total)
            </CardDescription>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Loan ID</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Borrower</th>
                    <th className="text-right p-2 font-light text-slate-600 dark:text-slate-400">Amount</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Status</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Current Status</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">App Date</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Closing Date</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Funding Date</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Lock Date</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Type</th>
                    <th className="text-left p-2 font-light text-slate-600 dark:text-slate-400">Branch</th>
                  </tr>
                </thead>
                <tbody>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700">
                <div className="text-sm text-slate-600 dark:text-slate-400 font-light">
                  Showing {(page - 1) * limit + 1} to {Math.min(page * limit, total)} of {total} loans
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
