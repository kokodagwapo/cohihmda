...
const BACKGROUND_TEMPLATES: {
  ...
  },
  {
    ...
    style: {
      background:
        "linear-gradient(180deg, #f97316 0%, #fb923c 30%, #fbbf24 100%)",
    },
  },
  {
    id: "forest",
    ...
    style: {
      background:
        "linear-gradient(160deg, #15803d 0%, #22c55e 50%, #86efac 100%)",
    },
  },
  {
    id: "lavender",
  ...
  },
  {
    ...
    style: {
      background:
        "linear-gradient(160deg, #fef3c7 0%, #fde68a 50%, #fcd34d 100%)",
    },
  },
  {
    id: "mesh",
    ...
    style: {
      background:
        "radial-gradient(at 40% 20%, #6366f1 0px, transparent 50%), radial-gradient(at 80% 0%, #8b5cf6 0px, transparent 50%), radial-gradient(at 0% 50%, #06b6d4 0px, transparent 50%)",
    },
  },
  {
    id: "minimal",
  ...
  },
  {
    ...
    items: [
      createLayoutItem(
        "t1-kpi",
        "kpi",
        { type: "kpi", label: "Total volume", value: 0, format: "currency" },
        { x: 0, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "t1-kpi2",
        "kpi",
        { type: "kpi", label: "Units", value: 0, format: "number" },
        { x: 3, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "t1-kpi3",
        "kpi",
        { type: "kpi", label: "Pull-through %", value: 0, format: "percent" },
        { x: 6, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "t1-section",
  ...
  },
  {
    ...
    items: [
      ...
      createLayoutItem(
        ...
        {
          type: "dashboard_section",
          sectionId: "closingFalloutForecast",
          title: "Closing & Fallout",
        },
        { x: 0, y: 2, w: 6, h: 2 },
      ),
      createLayoutItem(
  ...
  },
  {
    ...
    items: [
      createLayoutItem(
        ...
        {
          type: "text_block",
          content: "Campaign goals and KPIs",
  ...
  },
  {
    ...
    items: [
      createLayoutItem(
        "to-1",
        "kpi",
        { type: "kpi", label: "Pipeline", value: 0, format: "number" },
        { x: 0, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "to-2",
        "kpi",
        { type: "kpi", label: "Fallout %", value: 0, format: "percent" },
        { x: 3, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "to-3",
        "kpi",
        { type: "kpi", label: "Close rate", value: 0, format: "percent" },
        { x: 6, y: 0, w: 3, h: 1 },
      ),
      createLayoutItem(
        "to-4",
...
function normalizeTemplateItems(
  ...
  return pixelItems.map((item, index) => {
    const x =
      Math.max(0, Math.round((item.x - minX) * TEMPLATE_SCALE)) + start.x;
    const y =
      Math.max(0, Math.round((item.y - minY) * TEMPLATE_SCALE)) + start.y;
    const w = Math.max(
      TEMPLATE_MIN_SIZE.w,
      Math.round(item.w * TEMPLATE_SCALE),
    );
    const h = Math.max(
      TEMPLATE_MIN_SIZE.h,
      Math.round(item.h * TEMPLATE_SCALE),
    );
    return {
      ...item,
 ...
 */
const SECTION_TO_WIDGETS: Record<
  string,
  {
...
> = {
  ...
  executiveDashboard: {
    ...
    widgetIds: ["exec-dashboard-embed"],
  },
  loanDetail: {
 ...
 */
function migrateLoanDetailToWidgetGroup(
  items: CanvasLayoutItem[],
): CanvasLayoutItem[] {
  return items.map((item) => {
    if (
      ...
      return item;
    if ((item.payload as { sectionId?: string }).sectionId !== "loanDetail")
      return item;
...
export function WorkbenchCanvas({
  ...
  const [shareScope, setShareScope] = useState<"private" | "team" | "public">(
    "team",
  );
        ...
        case "generate_report": {
          ...
          if (!reportDef || !reportDef.slides?.length) {
            toast({
              ...
              description: "Report has no slides",
              variant: "destructive",
            });
            break;
          }
          toast({
            ...
            description: `Building ${reportDef.slides.length}-slide presentation`,
          });
          ...
          (async () => {
            try {
              ...
              toast({
                ...
                description: `${reportDef.title || "Report"} saved as ${fmt.toUpperCase()}`,
              });
            } catch (err: any) {
      ...
      for (const group of groups) {
        ...
        const groupPayload = {
          ...
          sectionType: (group.sectionType ||
            "company-scorecard") as import("@/stores/widgetSectionStore").SectionType,
          widgetIds: [] as string[],
          items: groupItems,
        ...
        const groupHeight = Math.max(
          500,
          widgetCount <= 4 ? 500 : 300 + widgetCount * 100,
        );
    ...
    (async () => {
      try {
        ...
        if (Array.isArray(content.layout)) {
          ...
          const shouldConvert =
            layoutVersion !== "freeform-v1" &&
            isLikelyGridLayout(content.layout);
        ...
        if (content.background && typeof content.background === "object")
          setCanvasBackground(content.background);
        if (Array.isArray(content.uploadsMeta)) setUploads(content.uploadsMeta);
        if (
          content.sourceInsight &&
          typeof content.sourceInsight === "object"
        ) {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            const shouldConvert2 =
              (content.layoutVersion as string | undefined) !== "freeform-v1" &&
              isLikelyGridLayout(content.layout ?? []);
  ...
  const applyBestFitLayout = useCallback(() => {
    ...
    const maxCols = Math.max(
      ...
      Math.min(
        3,
        Math.floor((canvasWidth + gap) / (DEFAULT_SECTION_SIZE.w + gap)),
      ),
    );
    const colCount = Math.min(Math.max(1, maxCols), n);
    const w = Math.max(
      280,
      Math.floor((canvasWidth - gap * (colCount - 1)) / colCount),
    );
    ...
    toast({
      ...
      description: "Dashboards arranged to fit.",
    });
  ...
  const applyColumnLayout = useCallback(() => {
    ...
    toast({
      title: "Layout applied",
      description: "Stacked in a single column.",
    });
  }, [items, canvasWidth, setItemsWithHistory, toast]);
  ...
  const applyRowLayout = useCallback(() => {
    ...
    toast({
      title: "Layout applied",
      description: "Arranged in a single row.",
    });
  }, [items, setItemsWithHistory, toast]);
  ...
  const applyMasonryLayout = useCallback(() => {
    ...
    const colCount = Math.max(
      ...
      Math.min(
        4,
        Math.floor((canvasWidth + gap) / (DEFAULT_WIDGET_SIZE.w + gap)),
      ),
    );
    const colHeights = Array.from({ length: colCount }).fill(0) as number[];
    const colWidth = Math.floor(
      (canvasWidth - gap * (colCount - 1)) / colCount,
    );
    const newItems = items.map((item) => {
    ...
    toast({
      title: "Layout applied",
      description: "Masonry grid for mixed heights.",
    });
  }, [items, canvasWidth, setItemsWithHistory, toast]);
  ...
  const addRichTextBlock = useCallback(() => {
    ...
    toast({
      title: "Rich text added",
      description: "Use the toolbar to style content.",
    });
  }, [addWidget, toast]);
  ...
  const addDashboardSection = useCallback(
    (sectionId: string, title: string) => {
      ...
      if (standalone) {
        ...
        toast({
          title: `${title} added`,
          description: "Widget added to canvas.",
        });
        return;
      }

      // ── Loan Detail: always use WidgetGroup (filter bar like Company Scorecard) ──
      const loanDetailLayout =
        sectionId === "loanDetail"
          ? {
              sectionType: "loan-detail" as const,
              widgetIds: ["loan-detail-table"] as string[],
            }
          : null;
      ...
      if (widgetLayout) {
        ...
        if (
          ...
          const chartRows = Math.ceil(chartCount / 2);
          const contentH =
            kpiRows * 80 + chartRows * 240 + tableCount * 300 + 20;
      ...
      const size =
        sectionId === "loanDetail"
          ? { w: 1100, h: 550 }
          : { w: DEFAULT_SECTION_SIZE.w, h: DEFAULT_SECTION_SIZE.h };
      setItemsWithHistory((prev) => {
      ...
      toast({
        title: "Dashboard added",
        description:
          "Drag to arrange. Use ⋮ → Bring to front / Send to back for layers.",
      });
    },
  ...
  useEffect(() => {
    ...
    setItems((prev) => {
      let y =
        prev.length === 0 ? 0 : Math.max(0, ...prev.map((i) => i.y + i.h)) + 24;
      const newItems: CanvasLayoutItem[] = [];
      pins.forEach((pin) => {
        const id = `widget-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
        const payload =
    ...
    if (pins.length > 0) {
      toast({
        ...
        description: `${pins.length} item(s) pinned. Open Canvas tab to see them.`,
      });
    }
    ...
    const handler = (e: Event) => {
      ...
      if (detail?.type && detail?.payload) {
        addWidget(
          detail.type,
          detail.payload,
          detail.size ?? { w: 360, h: 240 },
        );
        toast({
          title: "Added to workbench",
          description: "Widget from Cohi Chat",
        });
      }
    };
  ...
  const handleLogoChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = () => {
        ...
        addWidget(
          "image",
          { type: "image", src: dataUrl, alt: file.name },
          { w: 220, h: 140 },
        );
        toast({
          title: "Logo added",
          description: "Drag to position and resize.",
        });
      };
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = () => {
        ...
        toast({
          ...
          description: "Image applied to canvas.",
        });
      };
      reader.readAsDataURL(file);
    },
  ...
  const canvasContentHeight = Math.max(
    ...
    items.length === 0
      ? 400
      : Math.max(0, ...items.map((i) => i.y + i.h)) + 120,
  );
  ...
  const itemsMaxRight =
    items.length > 0 ? Math.max(0, ...items.map((i) => i.x + i.w)) : 0;
  ...
  const handleAiBackgroundSubmit = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/workbench/canvas/generate-background", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: aiBackgroundPrompt || "professional and clean",
        }),
      });
      if (!res.ok) throw new Error("Failed to generate");
      const data = await res.json();
      setAiBackgroundResult({
  ...
  const applyAiBackground = useCallback(() => {
    if (aiBackgroundResult) {
      setCanvasBackground({
        type: "template",
        value: aiBackgroundResult.templateId,
      });
      toast({
        title: "Background applied",
        description: aiBackgroundResult.suggestedDescription,
      });
...
<html lang="en">
  ...
  const handleEmailLink = useCallback(() => {
    if (!canvasId) {
      toast({
        title: "Save first",
        description: "Save the canvas to get a shareable link.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      title: "Link ready",
      description: "Email draft opened with link to the canvas.",
    });
  }, [
    canvasId,
    getShareUrl,
  ...
  const handleCopyShareLink = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Link copied",
        description: `Shareable ${shareScope} link copied to clipboard.`,
      });
    } catch {
  ...
  const handleToggleFavorite = useCallback(async () => {
    ...
    try {
      await api.request(
        `/api/workbench/canvases/${canvasId}/favorite${tenantQs}`,
        {
          method: "POST",
      ...
      toast({
        ...
        description: next
          ? "Canvas saved to your bookmarks."
          : "Canvas removed from bookmarks.",
      });
    } catch {
      toast({
        title: "Update failed",
        description: "Could not update bookmarks.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleCopyEmbedCode = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Paste into your website.",
      });
    } catch {
  ...
  const handleDownloadHtmlPage = useCallback(() => {
    ...
    toast({
      ...
      description: "Share the file as a standalone page.",
    });
  ...
  const handleSaveVisibility = useCallback(async () => {
    ...
    try {
      await api.request(
        `/api/workbench/canvases/${canvasId}/visibility${tenantQs}`,
        {
          method: "PUT",
          body: JSON.stringify({
      ...
      toast({
        ...
        description: `Canvas is now ${canvasVisibility}.`,
      });
  ...
  const handleClearCanvas = useCallback(() => {
    ...
    toast({
      ...
      description: "All blocks and annotations removed. Use Undo to restore.",
    });
  }, [
    setBothWithHistory,
    setUploads,
        ...
        <AlertDialog open={clearConfirmOpen} onOpenChange={setClearConfirmOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This removes all widgets, annotations, uploads, and resets the
                background. You can still Undo after clearing.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  className="mt-2"
                  onKeyDown={(e) =>
                    e.key === "Enter" && handleAiBackgroundSubmit()
                  }
                />
              </div>
              {!aiBackgroundResult ? (
                <Button
                  onClick={handleAiBackgroundSubmit}
                ...
                <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-3 space-y-2">
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    {aiBackgroundResult.suggestedDescription}
                  </p>
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setAiBackgroundResult(null)}
                    >
                      Try again
                    </Button>
                  </div>
