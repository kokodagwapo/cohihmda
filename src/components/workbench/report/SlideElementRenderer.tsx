...
function ChartElement({ config, scale }: { config: ChartElementConfig; scale: number }) {
  ...
  // Support multi-series via yKeys, fall back to single yKey
  ...
  // Compute max across all series for correct scaling
    ...
    <div className="w-full h-full flex flex-col p-2">
      {config.title && (
        <div
          ...
          style={{ fontSize: 11 * scale, color: '#1e293b' }}
        >
          {config.title}
        </div>
      )}
      <div className="flex-1 flex items-end gap-1 min-h-0">
        {config.chartType === 'pie' || config.chartType === 'donut' ? (
          <PiePreview data={data} colors={colors} nameKey={config.nameKey || xKey} valueKey={config.valueKey || yKey} />
        ) : config.chartType === 'line' || config.chartType === 'area' ? (
          <LinePreview data={data} xKey={xKey} yKey={yKey} color={colors[0]} />
        ) : (
          // Bar chart — supports multi-series (grouped bars)
          data.slice(0, 12).map((d, i) => (
            <div key={i} className="flex-1 flex flex-col items-center justify-end h-full min-w-0">
              ...
              <div className="text-[7px] text-slate-500 truncate w-full text-center mt-0.5" style={{ fontSize: 7 * scale }}>
      ...
      {data.length === 0 && (
        <div className="flex-1 flex items-center justify-center text-xs text-slate-400">
          {config.chartType || 'bar'} chart (no data yet)
        </div>
      )}
      {/* Legend for multi-series */}
      {data.length > 0 && yKeys.length > 1 && (
        <div className="flex items-center gap-2 mt-1 flex-wrap justify-center">
          {yKeys.map((k, i) => (
            <div key={k} className="flex items-center gap-0.5">
              <div className="w-2 h-2 rounded-sm" style={{ backgroundColor: colors[i % colors.length] }} />
              <span className="text-slate-500 truncate" style={{ fontSize: 6 * scale }}>{k}</span>
            </div>
          ))}
        </div>
      )}
...
function LinePreview({
  ...
  const points = data.map((d, i) => ({
    ...
    y: 100 - ((Number(d[yKey]) || 0) / maxVal) * 90,
  }));
