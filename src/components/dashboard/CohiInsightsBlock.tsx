...
export function CohiSectionCard(props: {
  ...
  return (
    <div className={`mb-4 p-5 rounded-xl ${s.bg}`}>
      <p
        className={`text-[13px] font-medium uppercase tracking-wider mb-3 flex items-center gap-2 ${s.label}`}
      >
        {s.display}
      </p>
      {section.items.map((item, idx) => {
        ...
        return (
          <div
            key={idx}
            className={`flex items-start gap-2.5 mb-2 last:mb-0 text-[15px] ${isCritical ? "text-rose-600 font-normal" : "text-slate-600"}`}
          >
...
export function CohiInsightsBlock(props: {
  ...
  const {
    ...
    loading = false,
    emptyText = "Load loan data to generate Cohi insights.",
    filterSections,
  } = props;

  const sectionsToShow =
    insights?.sections.filter(
      (s) => !filterSections || filterSections.includes(s.title),
    ) ?? [];

  return (
    <div
      className={`rounded-xl border overflow-hidden ${isDarkMode ? "bg-slate-900/30 border-slate-700" : "bg-white border-slate-200 shadow-sm"}`}
    >
      <div
        className={`px-5 py-4 border-b flex items-center justify-between ${isDarkMode ? "border-slate-700" : "border-slate-100"}`}
      >
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h3
              className={`font-medium text-[13px] sm:text-[14px] uppercase tracking-widest ${isDarkMode ? "text-slate-200" : "text-slate-700"}`}
            >
              Cohi
            </h3>
            <p
              className={`text-[11px] sm:text-[12px] font-light truncate ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
            >
              {subtitle}
            </p>
          </div>
        ...
        <div
          className={`text-[10px] px-2.5 py-1 rounded-md font-medium ${isDarkMode ? "bg-white/10 text-slate-300" : "bg-slate-50 text-slate-600 border border-slate-200"}`}
        >
