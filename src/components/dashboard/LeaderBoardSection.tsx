...
function CertificateSealIcon({ className, size = 32, id: sealId }: { className?: string; size?: number; id: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className={cn('flex-shrink-0', className)} width={size} height={size} aria-hidden>
      <defs>
        ...
        <filter id={`${sealId}-sh`} x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="1" stdDeviation="0.5" floodOpacity="0.3" />
        </filter>
      </defs>
      <polygon points={points.join(' ')} fill={`url(#${sealId})`} filter={`url(#${sealId}-sh)`} />
      <circle cx={r} cy={r} r={r * 0.72} fill="none" stroke="rgba(253,230,138,0.6)" strokeWidth={0.5} />
...
export const LeaderBoardSection = ({ dateFilter, selectedTenantId, hideAvatar = false }: LeaderBoardSectionProps) => {
  ...
  const [rankingMetric, setRankingMetric] = useState<'units' | 'volume' | 'turnTime' | 'pullThrough' | 'revenue'>('units');
  ...
  const getMetricValue = (leader: LeaderboardLeader, metric: 'units' | 'volume' | 'turnTime' | 'pullThrough' | 'revenue') => {
    switch (metric) {
      case 'units':
        return leader.loans;
      case 'volume':
  ...
  const sortByMetric = (data: LeaderboardLeader[], metric: 'units' | 'volume' | 'turnTime' | 'pullThrough' | 'revenue') => {
  ...
  const buildRankMap = (data: LeaderboardLeader[], metric: 'units' | 'volume' | 'turnTime' | 'pullThrough' | 'revenue') => {
  ...
  const leadersData = useMemo(
    () => sortByMetric(scopedData, rankingMetric).map((leader, idx) => ({ ...leader, rank: idx + 1 })),
  ...
  return <section ref={sectionRef} className="mt-4 sm:mt-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900/70 border border-slate-100 dark:border-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-5 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
        ...
        <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] sm:text-sm text-slate-600 dark:text-slate-300 font-light truncate">
              {getPeriodDisplayLabel()} · {scope === 'All' ? 'All branches' : scope === 'Branch' ? 'By branch' : 'By team'} · Ranked by {rankingMetric === 'units' ? 'Units' : rankingMetric === 'volume' ? 'Volume' : rankingMetric === 'turnTime' ? 'Turn-Time' : rankingMetric === 'pullThrough' ? 'Pull-through' : 'Revenue'}
            </p>
          </div>
        </div>
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="end">
              <div className="p-3 border-b border-slate-200 dark:border-slate-700">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Quick select a quarter or choose custom dates</p>
              </div>
              ...
              <div className="p-3 border-b border-slate-200 dark:border-slate-700 space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Select Quarter</span>
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {[1, 2, 3, 4].map((q) => {
                    ...
                    const isCurrentQuarter = customDateRange.start?.getTime() === qStart.getTime() && 
                    ...
                    return (
                      <Button
                        ...
                        className={cn(
                          "h-8 text-xs",
                          isCurrentQuarter && "bg-teal-600 hover:bg-teal-700 text-white",
                  ...
                  {(() => {
                    ...
                    const yearEnd = quarterYear < new Date().getFullYear() 
                      ? endOfYear(new Date(quarterYear, 0, 1))
                      : new Date(); // For current year, end at today
                    const isFullYear = customDateRange.start?.getTime() === yearStart.getTime() && 
                    ...
                    return (
                      <Button
                        ...
                        className={cn(
                          "h-8 text-xs",
                          isFullYear && "bg-teal-600 hover:bg-teal-700 text-white",
                          isFutureYear && "opacity-50"
                        )}
                        disabled={isFutureYear}
                        onClick={() => {
                          setCustomDateRange({ start: yearStart, end: yearEnd });
                          setPeriod('custom');
                          setCalendarOpen(false);
                        }}
                      >
              ...
              <div className="p-2">
                <p className="text-xs font-medium text-slate-600 dark:text-slate-400 px-1 mb-2">Or select custom range:</p>
                <Calendar
                  mode="range"
                  selected={{ 
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-white/40 dark:border-slate-700/50">
                <div className="text-center flex-1 min-w-0">
                  <p className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{leader.volume}</p>
                      ...
                      {others.map((leader) => (
                        ...
                        >
                          <td className="py-2.5 px-2 sm:px-3">
                            <p className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200 truncate max-w-[140px]">{leader.name}</p>
                            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 truncate max-w-[140px]">{leader.branch}</p>
                          </td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">{leader.loans.toLocaleString()}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm text-slate-500 dark:text-slate-400">{rankMaps.units.get(leader.id)}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">{leader.volume}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm text-slate-500 dark:text-slate-400">{rankMaps.volume.get(leader.id)}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">{leader.pullThru}%</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm text-slate-500 dark:text-slate-400">{rankMaps.pullThrough.get(leader.id)}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">{leader.cycleTime}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm text-slate-500 dark:text-slate-400">{rankMaps.turnTime.get(leader.id)}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm font-semibold text-emerald-600 dark:text-emerald-400">{leader.revenue}</td>
                          <td className="py-2.5 px-2 sm:px-3 text-right text-xs sm:text-sm text-slate-500 dark:text-slate-400">{rankMaps.revenue.get(leader.id)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-4 md:gap-6 lg:gap-8 flex-wrap">
          ...
          return (
            <button
              key={badge.key}
              type="button"
              onClick={() => setRankingMetric(badge.key as typeof rankingMetric)}
      ...
      <AnimatePresence>
        ...
        const rankBadgeClass = (r: number) => cn(
          "rounded-md px-1.5 py-0.5 text-[9px] font-semibold tabular-nums whitespace-nowrap inline-block",
          r === 1 ? "bg-amber-500/90 text-white" : r === 2 ? "bg-slate-400/90 text-white" : r === 3 ? "bg-orange-400/90 text-white" : "bg-slate-200/90 text-slate-600 dark:bg-slate-700/90 dark:text-slate-300"
        );
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 min-h-[3.5rem] text-center">
                      <div className="flex justify-center min-h-[1.25rem] items-center" aria-hidden>
                        <span className={rankBadgeClass(volumeRank)}>Rank #{volumeRank}</span>
                      </div>
                      <p className="text-lg font-light text-slate-900 dark:text-white tracking-tight">{leader.volume}</p>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">Volume</p>
                    </div>
                    <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 min-h-[3.5rem] text-center">
                      <div className="flex justify-center min-h-[1.25rem] items-center" aria-hidden>
                        <span className={rankBadgeClass(pullThroughRank)}>Rank #{pullThroughRank}</span>
                      </div>
                    ...
                    <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 min-h-[3.5rem] text-center">
                      <div className="flex justify-center min-h-[1.25rem] items-center" aria-hidden>
                        <span className={rankBadgeClass(turnTimeRank)}>Rank #{turnTimeRank}</span>
                      </div>
                    ...
                    <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 min-h-[3.5rem] text-center">
                      <div className="flex justify-center min-h-[1.25rem] items-center" aria-hidden>
                        <span className={rankBadgeClass(revenueRank)}>Rank #{revenueRank}</span>
                      </div>
