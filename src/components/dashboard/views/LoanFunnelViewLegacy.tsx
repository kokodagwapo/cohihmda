...
export const LoanFunnelViewLegacy = ({
  view,
  onViewChange,
  year,
  onYearChange,
  ...
  const formatFunnelValue = (val: number, viewType: typeof view): string => {
    ...
    if (viewType === 'units' || viewType === 'funnel' || viewType === 'bar') {
      ...
      if (val >= 1000) return val.toLocaleString(undefined, {
  ...
  const transformFunnelData = useMemo(() => {
    ...
    const getValue = (data: typeof effectiveFunnelData, key: keyof typeof effectiveFunnelData, viewType: typeof view): number => {
    ...
    // Estimate inquiries as 1.5x loans started (typical conversion)
    const inquiriesValue = getValue(effectiveFunnelData, 'loansStarted', view) * 1.5;
    const preApprovalValue = getValue(effectiveFunnelData, 'respaApp', view) * 0.95; // Estimate
    const lockedValue = getValue(effectiveFunnelData, 'originated', view) * 1.1; // Estimate
    ...
    const funnelDataPoints: FunnelDataPoint[] = [{
      ...
      color: '#9CA3AF',
      description: 'Initial customer contacts and leads'
    }, {
      id: 'started',
      label: 'Loans Started',
      value: getValue(effectiveFunnelData, 'loansStarted', view),
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'loansStarted', view), view),
      color: '#6B7280',
      ...
      color: '#475569',
      description: 'Customers pre-qualified for loans'
    }, {
      id: 'respa',
      label: 'Loans with RESPA Applications',
      value: getValue(effectiveFunnelData, 'respaApp', view),
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'respaApp', view), view),
      color: '#374151',
      ...
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'originated', view), view),
      color: '#F59E0B',
      ...
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'stillActive', view), view),
      color: '#3B82F6',
    ...
    const falloutDataPoints: FunnelDataPoint[] = [{
      ...
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'noRespaApp', view), view),
      color: '#E5E7EB',
      isFallout: true,
      volume: effectiveFunnelData.noRespaApp.volume,
      units: effectiveFunnelData.noRespaApp.units,
      lostRevenue: effectiveFunnelData.noRespaApp.lostRevenue || effectiveFunnelData.noRespaApp.revenue
    }, {
      id: 'withdrawn',
      label: 'Fallout - Withdrawn',
      value: getValue(effectiveFunnelData, 'falloutWithdrawn', view),
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'falloutWithdrawn', view), view),
      color: '#DC2626',
      isFallout: true,
      volume: effectiveFunnelData.falloutWithdrawn.volume,
      units: effectiveFunnelData.falloutWithdrawn.units,
      lostRevenue: effectiveFunnelData.falloutWithdrawn.lostRevenue || effectiveFunnelData.falloutWithdrawn.revenue
    }, {
      id: 'denied',
      label: 'Fallout - Denied',
      value: getValue(effectiveFunnelData, 'falloutDenied', view),
      valueDisplay: formatFunnelValue(getValue(effectiveFunnelData, 'falloutDenied', view), view),
      color: '#FEE2E2',
      textColor: '#991B1B',
      isFallout: true,
      volume: effectiveFunnelData.falloutDenied.volume,
      units: effectiveFunnelData.falloutDenied.units,
      lostRevenue: effectiveFunnelData.falloutDenied.lostRevenue || effectiveFunnelData.falloutDenied.revenue
    }];
    return {
      funnelDataPoints,
      falloutDataPoints
  ...
  const lostRevenueBreakdown = {
    noRespaApp: {
      ...
      recommendation: 'Manage these loans to ensure loan producers focus on conversion to closed loans.'
    },
    customerNo: {
      volume: effectiveFunnelData.falloutWithdrawn.volume,
      units: effectiveFunnelData.falloutWithdrawn.units,
      lostRevenue: effectiveFunnelData.falloutWithdrawn.lostRevenue || 0,
      recommendation: 'Consider the customer "No" rate by branch and originator.'
    },
    denied: {
      volume: effectiveFunnelData.falloutDenied.volume,
      units: effectiveFunnelData.falloutDenied.units,
      lostRevenue: effectiveFunnelData.falloutDenied.lostRevenue || 0,
      recommendation: 'Consider the loan type, credit box, and whether borrowers were referred to credit rehabilitation and other appropriate resources.'
    }
  };
  const totalLostRevenue = lostRevenueBreakdown.noRespaApp.lostRevenue + lostRevenueBreakdown.customerNo.lostRevenue + lostRevenueBreakdown.denied.lostRevenue;
  const totalLostVolume = lostRevenueBreakdown.noRespaApp.volume + lostRevenueBreakdown.customerNo.volume + lostRevenueBreakdown.denied.volume;
  const totalLostUnits = lostRevenueBreakdown.noRespaApp.units + lostRevenueBreakdown.customerNo.units + lostRevenueBreakdown.denied.units;
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-3 sm:p-4 md:p-6 border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-0.5 sm:gap-1 p-0.5 sm:p-1 bg-slate-100 dark:bg-slate-800/50 rounded-lg overflow-x-auto">
                  <button onClick={() => setActiveTab('company')} className={`px-2.5 py-1.5 sm:px-3 md:px-4 sm:py-2 text-[11px] sm:text-xs md:text-sm font-medium rounded-md transition-all whitespace-nowrap touch-manipulation ${isCompanyActive ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                    Company
                  </button>
                  <button onClick={() => setActiveTab('sales')} className={`px-2.5 py-1.5 sm:px-3 md:px-4 sm:py-2 text-[11px] sm:text-xs md:text-sm font-medium rounded-md transition-all whitespace-nowrap touch-manipulation ${isSalesActive ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                    Sales
                  </button>
                  <button onClick={() => setActiveTab('ops')} className={`px-2.5 py-1.5 sm:px-3 md:px-4 sm:py-2 text-[11px] sm:text-xs md:text-sm font-medium rounded-md transition-all whitespace-nowrap touch-manipulation ${isOpsActive ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                    Ops
                  </button>
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 p-0.5 sm:p-1 bg-slate-100 dark:bg-slate-800/50 rounded-lg overflow-x-auto">
              ...
              id: 'detail',
              label: 'Detail'
            }].map(v => <button key={v.id} onClick={() => onViewChange(v.id as any)} className={`px-2.5 py-1.5 sm:px-3 md:px-4 sm:py-2 text-[11px] sm:text-xs md:text-sm font-medium rounded-md transition-all whitespace-nowrap touch-manipulation ${view === v.id ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                  {v.label}
                </button>)}
            </div>
            ...
            {view === 'funnel' ? <div className="w-full overflow-hidden bg-white dark:bg-slate-800 p-4 sm:p-6">
                <FunnelVisualizationLegacy data={transformFunnelData.funnelDataPoints} falloutData={transformFunnelData.falloutDataPoints} />
              </div> : <div className="relative w-full">
                {/* Mobile Scroll Indicator - Blinking Arrow */}
                {showScrollIndicator && <div className="absolute right-2 top-1/2 -translate-y-1/2 z-20 sm:hidden pointer-events-none">
                  ...
                  <table className="w-full text-xs sm:text-sm">
                    ...
                    <thead>
                      <tr className="bg-gradient-to-r from-slate-100/90 to-slate-50/90 dark:from-slate-800/90 dark:to-slate-800/70 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-700/40">
                        ...
                        <th className="text-right py-3 px-3 sm:py-4 sm:px-5 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 whitespace-nowrap">Units %</th>
                        <th className="text-right py-3 px-3 sm:py-4 sm:px-5 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 whitespace-nowrap">Volume</th>
                        <th className="text-right py-3 px-3 sm:py-4 sm:px-5 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 whitespace-nowrap">Volume %</th>
                      </tr>
                    </thead>
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">Loans Started</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                          <span className="font-semibold text-slate-900 dark:text-white tabular-nums text-[10px] sm:text-xs">{effectiveFunnelData.loansStarted.units.toLocaleString()}</span>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">No RESPA Apps</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">RESPA Apps</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200 bg-gradient-to-r from-emerald-50/50 to-transparent dark:from-emerald-900/10 dark:to-transparent">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                            ...
                            <span className="font-semibold text-slate-900 dark:text-white text-[10px] sm:text-xs">Originated</span>
                            <span className="inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 text-[8px] sm:text-[9px] font-semibold uppercase tracking-wide">Success</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200 bg-gradient-to-r from-rose-50/40 to-transparent dark:from-rose-900/10 dark:to-transparent">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">Withdrawn</span>
                            <span className="inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400 text-[8px] sm:text-[9px] font-semibold uppercase tracking-wide">Lost</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200 bg-gradient-to-r from-amber-50/40 to-transparent dark:from-amber-900/10 dark:to-transparent">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">Denied</span>
                            <span className="inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 text-[8px] sm:text-[9px] font-semibold uppercase tracking-wide">Denied</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
                      ...
                      <tr className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-200 bg-gradient-to-r from-violet-50/40 to-transparent dark:from-violet-900/10 dark:to-transparent">
                        <td className="py-2 px-2 sm:py-3 sm:px-3" colSpan={2}>
                          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white text-[10px] sm:text-xs">Loans Still Active</span>
                            <span className="inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-400 text-[9px] sm:text-[10px] font-semibold uppercase tracking-wide">In Progress</span>
                          </div>
                        </td>
                        <td className="text-right py-2 px-2 sm:py-3 sm:px-3">
