  ...
  ({
    ...
    const fetchAiRecommendations = useCallback(
      (e?: React.MouseEvent) => {
        ...
        setAiLoading(true);
        setAiError(null);
        // Build URL with tenant_id query param for platform admins
        const url = selectedTenantId
          ? `/api/loans/${encodeURIComponent(
              loan.id
            )}/recommendations?tenant_id=${encodeURIComponent(
              selectedTenantId
            )}`
          : `/api/loans/${encodeURIComponent(loan.id)}/recommendations`;
        api
          .request<{ recommendations: string[] }>(url, { method: "GET" })
      ...
      [loan.id, selectedTenantId]
    );

    const lockVsMarketBucket =
      loan.interestLockVsMarketSignalStrength ??
      (() => {
        const delta =
          loan.marketChangeDelta ??
    ...
    const bucketBg = (b: number | null | undefined) => {
      if (b === null || b === undefined)
        return isDarkMode ? "bg-slate-700/50" : "bg-slate-100";
    ...
    const hasAnySignal =
      loan.creditMetricsSignalStrength != null ||
      loan.loanCharacteristicsSignalStrength != null ||
      loan.timeInMotionSignalStrength != null ||
      loan.mloAeFalloutProneSignalStrength != null ||
      loan.loPullthroughSignal != null ||
      lockVsMarketBucket != null;

    const signalItems: { label: string; value: number | null }[] = [
      {
        label: "Credit Metrics",
        value: loan.creditMetricsSignalStrength ?? null,
      },
      {
        label: "Loan Characteristics",
        value: loan.loanCharacteristicsSignalStrength ?? null,
      },
      {
        label: "Time in Motion",
        value: loan.timeInMotionSignalStrength ?? null,
      },
      {
    ...
    return (
      <>
        <div
          className={`flex items-start justify-between gap-3 mb-2 sm:mb-3 ${
            !showTapForDetails ? "mt-8" : ""
          }`}
        >
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
            <div className="min-w-0 flex-1">
              <p
                className={`font-medium text-[13px] sm:text-sm tracking-tight break-words ${
              ...
              {onSelectOfficer ? (
                ...
                >
                  ...
                  >
                    {loan.officer || "Unknown LO"}
                  </span>
                  {loan.officerTtsScore != null &&
                    !Number.isNaN(loan.officerTtsScore) && (
                      <span
                        className={`flex-shrink-0 text-[13px] sm:text-sm font-medium ${
                          isDarkMode ? "text-slate-400" : "text-slate-500"
                        }`}
                      >
                        {loan.officerTier === "top"
                          ? "Top Tier"
                          : loan.officerTier === "second"
                          ? "Second Tier"
                          : "Bottom Tier"}{" "}
                        – {Math.round(loan.officerTtsScore)}
                      </span>
                    )}
                </button>
              ) : (
                <p className="text-[13px] sm:text-sm font-medium mt-0.5">
                  <span
                    className={isDarkMode ? "text-slate-300" : "text-slate-900"}
                  >
                    MLO/AE:{" "}
                  </span>
                  <span
                  ...
                  {loan.officerTtsScore != null &&
                    !Number.isNaN(loan.officerTtsScore) && (
                      <span
                        className={`ml-1 text-[13px] sm:text-sm font-medium ${
                          isDarkMode ? "text-slate-400" : "text-slate-500"
                        }`}
                      >
                        {loan.officerTier === "top"
          ...
          <div className="mb-3 pt-2.5 pb-2 border-t border-transparent">
            ...
            <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
              {signalItems.map(({ label, value }) => (
                <div
                  key={label}
                  className={`rounded-lg px-1.5 py-2 sm:px-2 sm:py-2.5 text-center min-w-0 break-words ${bucketBg(
                    value
                  )}`}
                >
                  <p
                    className={`text-[9px] sm:text-[10px] font-medium uppercase tracking-wide break-words mb-0.5 ${
                      isDarkMode ? "text-slate-400" : "text-slate-500"
                    }`}
                  >
                    {label}
                  ...
                  <p
                    className={`text-sm sm:text-base font-semibold break-words ${bucketText(
                      value
                    )}`}
                  >
                    {value != null ? value : "—"}
                ...
                <div>
                  ...
                  {loan.lockDate != null && loan.lockDate !== "" ? (
                    loan.lockExpirationDate ? (
                      <p
                        className={`font-medium text-sm ${getExpirationDateColorClass(
                          loan.lockExpirationDate,
                          isDarkMode
                        )}`}
                      >
        ...
        {showRiskBreakdown &&
          (() => {
            ...
            if (loan.ficoScore) {
              if (loan.ficoScore >= 700)
                localSuccesses.push(
                  `Strong credit profile (FICO ${loan.ficoScore})`
                );
              else if (loan.ficoScore >= 620)
                localWarnings.push(
                  `Credit needs monitoring (FICO ${loan.ficoScore})`
                );
              else
                localCriticals.push(
                  `High-risk credit profile (FICO ${loan.ficoScore})`
                );
            }
            const ltvNum = loan.ltvRatio != null ? Number(loan.ltvRatio) : null;
            if (ltvNum != null && !Number.isNaN(ltvNum)) {
              if (ltvNum <= 80)
                localSuccesses.push(
                  `Healthy equity position (LTV ${ltvNum.toFixed(0)}%)`
                );
              else if (ltvNum <= 95)
                localWarnings.push(
                  `Elevated LTV may require PMI (${ltvNum.toFixed(0)}%)`
                );
              else
                localCriticals.push(
                  `Very high LTV indicates minimal equity (${ltvNum.toFixed(
                    0
                  )}%)`
                );
            }
            const dtiNum = loan.dtiRatio != null ? Number(loan.dtiRatio) : null;
            if (dtiNum != null && !Number.isNaN(dtiNum)) {
              if (dtiNum <= 36)
                localSuccesses.push(
                  `Manageable debt load (DTI ${dtiNum.toFixed(0)}%)`
                );
              else if (dtiNum <= 43)
                localWarnings.push(
                  `DTI approaching threshold (${dtiNum.toFixed(0)}%)`
                );
              else
                localCriticals.push(
                  `DTI exceeds QM threshold (${dtiNum.toFixed(0)}%)`
                );
            }
            ...
            return (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                {successes.length > 0 && (
                  <div
                    className={`p-3 rounded-xl border-l-4 bg-sky-50 dark:bg-sky-900/20 border-sky-400`}
                  >
                    <p
                      className={`text-[10px] font-semibold uppercase tracking-wider mb-2 text-sky-600 dark:text-sky-400`}
                    >
                      Successes
                    </p>
                    <ul
                      className={`text-[12px] space-y-1 text-sky-700 dark:text-sky-300`}
                    >
                      {successes.map((s, i) => (
                        <li key={i}>• {s}</li>
                      ))}
                ...
                {warnings.length > 0 && (
                  <div
                    className={`p-3 rounded-xl border-l-4 bg-amber-50 dark:bg-amber-900/20 border-amber-500`}
                  >
                    <p
                    ...
                    <ul
                      className={`text-[12px] space-y-1 text-amber-700 dark:text-amber-300`}
                    >
                      {warnings.map((w, i) => (
                        <li key={i}>• {w}</li>
                      ))}
                ...
                {criticals.length > 0 && (
                  <div
                    className={`p-3 rounded-xl border-l-4 bg-red-50 dark:bg-red-900/20 border-red-500`}
                  >
                    <p
                      className={`text-[10px] font-semibold uppercase tracking-wider mb-2 text-red-600 dark:text-red-400`}
                    >
                      Critical
                    </p>
                    <ul
                      className={`text-[12px] space-y-1 text-red-700 dark:text-red-300`}
                    >
                      {criticals.map((c, i) => (
                        <li key={i}>• {c}</li>
                      ))}
        ...
        {showTapForDetails && (
          <div
            className={`mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t flex items-center justify-between ${
